...
On Mac/Linux:
```bash
django-tutorial$ virtualenv venv
django-tutorial$ source ./venv/bin/activate
(venv)$
```
On Windows:
```bash
django-tutorial$ virtualenv venv
django-tutorial$ ./venv/Scripts/activate.bat
(venv) django-tutorial$
```
...
```bash
(venv) django-tutorial$ django-admin startproject core .
```
...
```bash
(venv) django-tutorial$ python manage.py runserver
```
...
```bash
(venv) django-tutorial$ django-admin  startapp example
```
...
```bash
(venv) django-tutorial$ pip install django-theme-material-kit
```
...
```bash
(venv) django-tutorial$ # Set up the database
(venv) django-tutorial$ python manage.py makemigrations
(venv) django-tutorial$ python manage.py migrate
(venv) django-tutorial$ python manage.py createsuperuser # Create the superuser
(venv) django-tutorial$ python manage.py runserver # Start the application (development mode)
```

Opening the URL `http://127.0.0.1:8000` from your browser you will see a new application that has been installed. and opening the route `http://127.0.0.1:8000/example` opens the previous application that was created.
...
```bash
(venv) django-tutorial$ pip install whitenoise[brotli]
```
...
```bash
(venv) django-tutorial$ pip install gunicorn
```
...
```bash
(venv) django-tutorial$ chmod a+x build.sh
```

* Create a new repository on GitHub, initialize the project directory and add the GitHub repository as the remote URL

```bash
(venv) django-tutorial$ git init -b main
(venv) django-tutorial$ git remote add origin <link-to-github-repo>
(venv) django-tutorial$ git add .
(venv) django-tutorial$ git commit -m "<COMMIT-MESSAGE>"
(venv) django-tutorial$ git push origin main
```
...
```
This uses a python base image and copies the application code into the `app` folder in our docker container. The `ENV` command creates environmental variables, these variables can be overwritten when building the docker container.

* Run the command below on your terminal
```bash
(venv) django-tutorial$ docker build -t django-app:1.0
```
...
```bash
(venv) django-tutorial$ docker run -d -p 8000:8000 django-app:1.0
```
